...
@section('subtitle', 'Vendor Accounts · Supervisor View')
        ...
        <div class="flex items-start justify-between">
            <div>
                <p class="text-slate-400 font-semibold"
                   style="font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.07em;">Total Vendors</p>
            ...
            <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                ...
                <i class="bi bi-shop-window" style="font-size: 17px; color: #059669;"></i>
            </div>
        </div>
    </div>
...
<div class="bg-white rounded-xl border border-slate-100 overflow-hidden"
    ...
    <div class="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h2 class="text-slate-700 font-bold" style="font-size: 13.5px;">Vendor Accounts</h2>
    ...
    {{-- Search & Filter Bar --}}
    <form method="GET" action="{{ route('supervisor.vendors.index') }}" class="px-5 py-4 border-b border-slate-100">
        <div class="flex flex-col sm:flex-row gap-3 items-end">
            ...
            <div class="flex-1">
                ...
                <div style="position: relative;">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, username, or stall..."
            ...
            <button type="submit"
                    ...
                    style="background: #059669; color: white; font-size: 13px;"
                    onmouseover="this.style.background='#047857'"
                    onmouseout="this.style.background='#059669'">
    ...
    <div class="flex flex-col items-center justify-center py-16 text-center">
        <div class="w-14 h-14 rounded-full flex items-center justify-center mb-4"
             style="background: #f8fafc;">
            <i class="bi bi-shop-window" style="font-size: 26px; color: #cbd5e1;"></i>
        </div>
        ...
        <table class="w-full" style="border-collapse: collapse; min-width: 700px;">
            <thead>
                <tr style="background: #f8fafc; border-bottom: 1px solid #f1f5f9;">
                    ...
                    <th class="text-left px-5 py-3 text-slate-400 font-semibold"
                        style="font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.07em;">Stall #</th>
                    <th class="text-left px-5 py-3 text-slate-400 font-semibold"
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #f8fafc; transition: background 0.1s;"
                    ...
                    <td class="px-5 py-4 text-slate-400" style="font-size: 12px;">{{ $vendors->firstItem() + $loop->index }}</td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                                 style="background: #f0fdf4;">
                                <i class="bi bi-person-fill" style="font-size: 13px; color: #059669;"></i>
                            </div>
                    ...
                    <td class="px-5 py-4">
                        @if($vendor->vendorProfile)
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md font-semibold font-mono"
                                  style="font-size: 11.5px; background: #ecfdf5; color: #065f46; border: 1px solid #a7f3d0;">
                                <i class="bi bi-shop" style="font-size: 10px;"></i>
                                {{ $vendor->vendorProfile->stall_number }}
                            </span>
                        @else
                            <span class="text-slate-300" style="font-size: 12px;">— unassigned —</span>
                        @endif
                    </td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <button onclick="openEditModal(
                                        ...
                                        '{{ $vendor->username }}',
                                        '{{ addslashes($vendor->vendorProfile?->stall_number ?? '') }}'
                                    )"
                            ...
                            {{-- Toggle Status --}}
                            <form method="POST"
                                  action="{{ route('supervisor.vendors.toggle', $vendor) }}"
    ...
    <div class="px-5 py-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <p class="text-slate-500" style="font-size: 12px;">
            Showing <span class="font-semibold text-slate-700">{{ $vendors->firstItem() }}</span> to
            <span class="font-semibold text-slate-700">{{ $vendors->lastItem() }}</span> of
            <span class="font-semibold text-slate-700">{{ $vendors->total() }}</span> vendors
        </p>
        ...
        <div class="flex items-center gap-2">
            ...
            @else
                <a href="{{ $vendors->previousPageUrl() . (request('search') ? '&search=' . request('search') : '') . (request('status') ? '&status=' . request('status') : '') }}"
            ...
            @if ($vendors->hasMorePages())
                <a href="{{ $vendors->nextPageUrl() . (request('search') ? '&search=' . request('search') : '') . (request('status') ? '&status=' . request('status') : '') }}"
                   class="inline-flex items-center justify-center w-8 h-8 rounded-lg transition-colors"
                   style="border: 1px solid #e2e8f0; color: #475569; background: white;"
                   onmouseover="this.style.background='#f8fafc'; this.style.borderColor='#cbd5e1'"
                   onmouseout="this.style.background='white'; this.style.borderColor='#e2e8f0'">
...
<div id="addModal"
    ...
    <div class="modal-box bg-white rounded-2xl w-full max-w-md overflow-hidden"
        ...
        {{-- Modal form --}}
        <form method="POST" action="{{ route('supervisor.vendors.store') }}" class="px-6 py-5">
            @csrf
...
<div id="editModal"
    ...
    <div class="modal-box bg-white rounded-2xl w-full max-w-md overflow-hidden"
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h3 class="text-slate-800 font-bold" style="font-size: 15px;">Edit Vendor Account</h3>
                <p class="text-slate-400" style="font-size: 11.5px; margin-top: 1px;">
                    Update vendor details, stall, and credentials
                </p>
            </div>
        ...
        <form id="editForm" method="POST" action="" class="px-6 py-5">
            @csrf
            @method('PUT')
            <input type="hidden" id="editUserId" name="_edit_user_id" value="">
            ...
            <div class="mb-4 rounded-xl px-4 py-3" style="background: #fef2f2; border: 1px solid #fecaca;">
                <div class="flex items-start gap-2.5">
                    ...
                    <p style="font-size: 13px; color: #991b1b; line-height: 1.5;">
                        ...
                        <strong id="deleteVendorName" style="font-weight: 700;"></strong>.
    ...
    <div class="modal-box bg-white rounded-2xl w-full max-w-sm overflow-hidden"
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-400" style="font-size: 11.5px; margin-top: 1px;">
                    The vendor will lose access until reactivated.
                </p>
            </div>
            ...
            <div class="mb-5 rounded-xl px-4 py-3" style="background: #fffbeb; border: 1px solid #fde68a;">
                <div class="flex items-start gap-2.5">
                    ...
                    <p style="font-size: 13px; color: #92400e; line-height: 1.5;">
                        <strong id="deactivateVendorName" style="font-weight: 700;"></strong>
                        will no longer be able to log in or submit inventory entries.
    ...
    <div class="modal-box bg-white rounded-2xl w-full max-w-sm overflow-hidden"
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-400" style="font-size: 11.5px; margin-top: 1px;">
                    The vendor will regain access to the system.
                </p>
            </div>
            ...
            <div class="mb-5 rounded-xl px-4 py-3" style="background: #ecfdf5; border: 1px solid #a7f3d0;">
                <div class="flex items-start gap-2.5">
                    ...
                    <p style="font-size: 13px; color: #065f46; line-height: 1.5;">
                        <strong id="activateVendorName" style="font-weight: 700;"></strong>
                        will be able to log in and submit inventory entries again.
...
<script>
    ...
    function openModal(id) {
        ...
        el.querySelector('.modal-box').offsetHeight;
    ...
    function openEditModal(id, name, username, stallNumber) {
        document.getElementById('editForm').action = `/supervisor/vendors/${id}`;
    ...
    function openDeactivateModal(id, name) {
        document.getElementById('deactivateForm').action = `/supervisor/vendors/${id}/toggle`;
        document.getElementById('deactivateVendorName').textContent = name;
        openModal('deactivateModal');
    }

    // ── Open Activate modal ───────────────────────────────────────
    function openActivateModal(id, name) {
        document.getElementById('activateForm').action = `/supervisor/vendors/${id}/toggle`;
        document.getElementById('activateVendorName').textContent = name;
